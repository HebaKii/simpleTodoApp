...
$(document).ready(function () {
	...
	$ul.on('change', '.complete', complete);
	$ul.on('click', '.deletetask', deletetask);
	...
	function deletetask(event) {
		var deletedTask = $(event.target).closest('li');
		var d = $ul.find('li').index(deletedTask);
		
		app.tasks.splice(d,1);
		localStorage.setItem('app', JSON.stringify(app));
		$(this).parent().remove();
